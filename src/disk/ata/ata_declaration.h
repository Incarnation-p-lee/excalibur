...
static inline s_ata_device_info_t * ata_device_info(void);
static inline uint16 ata_device_cylinder_read(uint16 status_port, uint16 port_low, uint16 port_high);
static inline uint32 ata_device_info_type(s_ata_device_info_t *dev_info);
static inline void ata_device_drive_set(uint16 port, uint8 val);
static inline void ata_device_info_print(s_ata_device_info_t *dev_info);
